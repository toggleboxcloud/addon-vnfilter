...
REAL_MANIFEST="$REPO_ROOT/manifests/cloud-7.4.1.tsv"
failed=0
...
e2e() {
    local dest="$1"; shift
    printf '%s\n' "$@" > "$FAKE/manifests/cloud-7.4.1.tsv"
...
if [[ $EUID -eq 0 ]] && id -u oneadmin >/dev/null 2>&1; then
    printf '%s\n' "$GOOD_ROW" "$LINK_ROW" > "$FAKE/manifests/cloud-7.4.1.tsv"
...
D3="$TMP/d3"
rm -f "$FAKE/manifests/cloud-7.4.1.tsv"
